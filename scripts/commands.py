...
@divinity.command(aliases=["masschannels"])
async def masschannel(ctx):
    await ctx.message.delete()
    for _i in range(500):
        try:
            await ctx.guild.create_text_channel(name=f"virtualworld on top")
        except:
            return

@divinity.command(aliases=["massroles"])
async def massrole(ctx):
    await ctx.message.delete()
    for _i in range(500):
        await ctx.guild.create_role(name=(f"/gamemode creative"), color=RandomColor())

@divinity.command(aliases=["delchannel"])
...
@divinity.command(aliases=["delrole"])
...
async def countdown(ctx, length=None):
    ...
    try:
        ...
        await ctx.send(f"Please give a valid number for your countdown.")
        return
    if t < 1:
        await ctx.send(f"Please give a valid length for the countdown.")
        return
    global do_loops
    do_loops = True
    ...
    await msg.edit(content=f":alarm_clock: Countdown: Countdown completed!")